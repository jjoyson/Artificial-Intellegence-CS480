        ...
        if players is None:
            ...
            while self.players[0] == None:
                ...
                if choice == "Human" or choice.lower() == "h":
                    ...
                    search = str(input("What serach do u want to run ? (AlphaBeta or anything else = Minimax) "))
                    evaluation = str(input("What eval function do u want to run ? (Modified or anything else = Basic) "))
                    if(search != "AlphaBeta"):
                        search = "Minimax"
                    if(evaluation != "Modified"):
                        evaluation = "Basic"
                    self.search = search
                    self.evaluation[0] = evaluation
                    self.players[0] = AIPlayer(name, self.colors[0], diff+1, search, evaluation)
                else:
            ...
            while self.players[1] == None:
                ...
                if choice == "Human" or choice.lower() == "h":
                    ...
                    self.players[1] = AIPlayer(name, self.colors[1], diff+1, search, evaluation)
                else:
                    print("Invalid choice, please try again")
    ...
    def verticalCheck(self, row, col):
        ...
        if consecutiveCount >= 5:
            ...
            if self.players[0].color.lower() == self.board[row][col].lower():
                ...
                self.evaluate = self.evaluation[0]
            else:
                self.winner = self.players[1]
                self.evaluate = self.evaluation[1]
